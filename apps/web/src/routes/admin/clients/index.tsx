...
/**
 * `/admin/clients` — the registered applications.
 *
 ...
 *
 * **Create and edit are pages**: `/admin/clients/new` and
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // pages of their own — but the order is the house one and a
...
function ClientsPage() {
  ...
  return (
    <AdminShell
      ...
      wideDescription
      actions={
        // A link, not a dialog trigger. `/admin/clients/new` is
            ...
            <TableHeader>
              <TableRow>
                {/* The actions column carries no visible heading — a menu
                    ...
                    cannot name. First column and pinned (`sticky
            ...
            <TableBody>
              {clients.map((client) => (
                <TableRow key={client.clientId}>
                  ...
                  <TableCell>
                    {/* the name is the way in. Edit used to exist
                        only inside the per-row `⋯`, so the whole point of an
                        addressable record was reachable only by opening a
                        menu — and the `⋯` is the first, pinned column, so linking the second restores a normal
                  ...
                  {/* One affordance for both answers. It was a Badge for yes
                      ...
                      (finding 10) — and it named `skipConsent`, so
                  ...
                  {/* Status, and only status. The four actions
