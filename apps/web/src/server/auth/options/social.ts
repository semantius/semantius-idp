/**
 * Social providers from configuration alone.
 *
 ...
 *   sign-in succeeds only for identities that already exist.
 * - **`given_name` / `family_name` → `firstName` / `lastName`**,
 *   and the display name re-derived from them in `site.nameFormat` order
 * — but only when the provider actually shipped a part. A provider
 ...
 * options, which is what makes `(providerId, accountId)` the only
 * identity.
 */
...
export function buildSocialProviders(
  ...
  for (const [providerId, provider] of entries) {
    providers[providerId] = {
      ...
      // with sign-up off, an unknown identity is refused rather
      // than silently registered.
      ...(config.file.signUp.enabled ? {} : { disableImplicitSignUp: true }),

      // `syncProfile` is what Better Auth calls
      ...
      // name mapping, applied on create and on
...
/**
 * Whether a provider-supplied e-mail is allowed to register (with
 * the domain allow-list, plus the per-provider `allowedEmailDomains`).
...
/** Trimmed and lower-cased, everywhere an address is handled. */
