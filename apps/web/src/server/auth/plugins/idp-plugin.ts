/**
 * The IdP's own Better Auth plugin.
 *
 ...
 * same generator as `user` and `session` and are covered by the spec drift gate
 ...
 * live here — as endpoints rather than page-only logic, because the spec says
...
/**
 * Append-only audit trail. No secrets are ever stored: `metadata`
...
/**
 * `pending_authorization` — a table that is declared, migrated, swept, and
 * **never written**.
 *
 ...
 * signed request is bound to a session only by the provider's own freshness
 * check, not by this table**; a reviewer reading the columns below and
 * concluding the pending request is session-bound would be wrong, which is
 * why this comment exists.
 *
 * The declaration stays because removing it is a migration for no gain: no
 * row is ever inserted, and the cleanup job's sweep of it is a no-op.
 */
...
export interface IdpPluginOptions {
  ...
  mailer?: Mailer
  /**
   * The audit middleware. Registered through this plugin so it runs
   * *after* every other plugin's `after` hook — see the comment at the
   * bottom of {@link idpPlugin}.
   */
  afterHook?: AuthMiddleware
  /**
   * the invariants that wrap Better Auth's own admin endpoints.
  ...
  /** what a completed admin mutation still owes. */
  adminAfterHook?: AuthMiddleware
  /**
   * the administrative endpoints, built in `admin/endpoints.ts`.
...
export function idpPlugin(options: IdpPluginOptions): BetterAuthPlugin {
  ...
  const approveUser = createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      await audit?.record({
        ...
        // No address here, and no way to pass one: the writer reads the one
        // the edge resolved under `server.trustProxy`, and `X-Forwarded-For`
        // is attacker-controlled at the left.
        userAgent: ctx.request?.headers.get("user-agent"),
        metadata: { previousStatus: user.status },
      })

      // the approval e-mail links to /login. It deliberately does
  ...
  const rejectUser = createAuthEndpoint(
    "/idp/reject-user",
    {
      ...
      body: z.object({
        ...
        /** Whether to tell them. The rejection e-mail is optional. */
    ...
    async (ctx) => {
      ...
      // the row stays, so the address remains reserved; and any
  ...
  return {
    ...
    // The audit trail runs **here** rather than as `options.hooks.after`,
...
export type AuditAction =
  | "signin.success"
  | "signin.failure"
  // an account somebody made for somebody else — an administrator, or
  // the first-run wizard. `signup.created` stays what it has always meant:
  // a self-service registration.
  | "user.created"
  | "signup.created"
  // a registration refused because the address already has an
  // account. Recorded by `/signup` and nowhere else, because it is the one
  // place the refusal is hidden from the response and the trail is
  // where it has to be visible instead.
  | "signup.duplicate"
  | "signup.approved"
  | "signup.rejected"
  | "email.verified"
  ...
  | "client.reconciled"
  | "client.created"
  // an admin-registered client is editable, and rotating its secret is
  ...
  | "client.disabled"
  | "keys.rotated"
  // the API gateways of FR-GW. `reconciled` is the boot sweep (ids and
  ...
  | "gateway.deleted"
  | "gateway.disabled"
  // one row per `/admin/database` execution, success or not. The console
  // can read everything at rest, so "an administrator ran SQL" is exactly the
  // kind of event the audit trail exists to make undeniable -- and the statement itself
