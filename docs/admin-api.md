...
The entry is validated by the **same schema `oauth_clients.json` is**, so a
...
| `CLIENT_MANAGED_BY_FILE` | That client comes from `oauth_clients.json`. Edit the file and restart. |
| `CLIENT_ALREADY_EXISTS` | A client with that id is already registered. |
| `INVALID_CLIENT_DEFINITION` | The entry does not satisfy the `oauth_clients.json` schema; the message names what. |
