...
**There is a second way**: register one from `/admin/clients`. The
...
server and shown once, and **every field except the client id can be edited
from the same page**, along with disabling, removing, and rotating
the secret. The id is fixed because it is the key the tokens, the consents and
the audit trail all reference. An edit is a **full replace** of what the form
shows: a confidential client that stays confidential keeps its existing secret
working, changing a single-page or native application into a web one issues a
secret and shows it once, and going the other way discards it. Only that change
of kind revokes anything — a renamed application or a corrected redirect URI
does not sign anybody out. Use the file when the deployment should be
reproducible from configuration, and the page when it should not need one.
...
is `none`, and PKCE is mandatory. `web` is confidential: a **generated** `clientSecret` — `openssl rand -base64 48`
— of at least 32 characters and at least 8 distinct ones, sent with
`client_secret_basic` by default. The file is refused below that floor, and a
placeholder word (`example`, `change-me`, …) is a start-up warning: the secret
is stored as an unsalted digest, so its entropy is the whole protection.
...
operator wrote the entry down and can read the default beside it — asking the user to approve an application their
...
| `clientSecret` | — | Required for `web`, forbidden otherwise. ≥ 32 characters and ≥ 8 distinct ones (a placeholder word such as `example` is a start-up warning), and a `${env:…}` placeholder in production. |
