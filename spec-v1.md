...
**CFG-3 — Precedence & env.** Effective value = config file (after substitution) → fallback env (only when the key is absent from the file: `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `DATABASE_URL`) → schema default. Env-only bootstrap variables (never in files): `IDP_CONFIG_DIR`, `HOST`, `PORT`, `LOG_LEVEL`, `LOG_FORMAT`, `IDP_MIGRATE_ON_BOOT`, `IDP_EMAIL_TRANSPORT`, `IDP_EMAIL_CAPTURE_DIR` (**D30** — a config-file key would be a copy-pasteable way to make a production deployment swallow every e-mail). There is **no** generic `IDP__SECTION__KEY` override mechanism.
...
**OPS-6 — Operator CLI.** `idp config validate` · `idp migrate` · `idp reconcile-clients` · `idp reset-admin [email]` (lockout recovery, FR-ADMIN-1 / **D42**) · `idp rotate-keys` · `idp cleanup` · `idp version` — all runnable as `docker run <image> idp <cmd>` against the same config folder/env.

**OPS-7 — Reference deployment.** `docker-compose.yml`: pinned Postgres (16/17), named volume, healthchecks, `depends_on: condition: service_healthy`, `./config:/config:ro`, `env_file`, `secrets:` — with `IDP_CONFIG_DIR`, `DATABASE_URL`, `DIRECT_DATABASE_URL` and `IDP_SCHEMA_NAME` pinned in `environment`, which wins over `env_file`, so a developer's host values cannot redirect the container (**D43**); optional **Caddy** profile (automatic HTTPS; Caddyfiles for host-root **and** sub-path incl. the origin-root RFC 8414 route). No Mailpit (Resend-only); dev/test e-mail uses the capture transport.
