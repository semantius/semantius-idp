...
**FR-OIDC-4 — Endpoints.** `/oauth2/authorize`, `/oauth2/token`, `/oauth2/userinfo`, `/oauth2/introspect` (client auth; answers only for the caller's own tokens unless the client is flagged `resourceServer: true`), `/oauth2/revoke` (RFC 7009; always 200), `/oauth2/end-session`. `/oauth2/register` and the provider's own client/consent CRUD endpoints are unreachable (FR-OIDC-2). Client registration for administrators is this deployment's own `POST /api/auth/idp/create-client` · `/idp/delete-client` · `/idp/set-client-disabled` (**D50**): admin-gated, fresh-session gated at the route, validated with the `oauth_clients.json` entry schema, secret server-generated (≥ 32 chars) and returned once, hashed at rest, audit-logged, taken under the reconcile advisory lock, refusing a duplicate id and refusing to mutate a `user_id IS NULL` row. Unknown `client_id` or unregistered `redirect_uri` renders a branded error page and **never redirects**.
...
**CFG-3 — Precedence & env.** Effective value = config file (after substitution) → fallback env (only when the key is absent from the file: `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `DATABASE_URL`) → schema default. `DATABASE_URL_ADMIN` is the fallback env for `database.directUrl` (renamed from `DIRECT_DATABASE_URL`, **D48** — a clean break, no alias). Env-only bootstrap variables (never in files): `IDP_CONFIG_DIR`, `HOST`, `PORT`, `LOG_LEVEL`, `LOG_FORMAT`, `IDP_MIGRATE_ON_BOOT`, `IDP_EMAIL_TRANSPORT`, `IDP_EMAIL_CAPTURE_DIR` (**D30** — a config-file key would be a copy-pasteable way to make a production deployment swallow every e-mail). There is **no** generic `IDP__SECTION__KEY` override mechanism.
...
**SEC-6 — Audit log.** Append-only `audit_log` table (actor, target, action, outcome, ip, ua, requestId, metadata, createdAt) **and** stdout events for: sign-in success/failure, sign-up, verification, approval/rejection, ban/unban, role change, password change/reset (requested/completed), session revocation, 2FA enrol/disable/reset, API-key create/revoke/failure, impersonation start/stop, consent grant/revoke, token issuance (`client_id`, `sub`, scopes), token revocations, client reconciliation diffs, **client create/delete/disable from the admin UI** (**D50**), **admin profile edits** (`user.profile_changed`), key rotation, `social.profile_conflict`. An API-key event's target is always the **key** (`targetType: "apikey"`), with the owner in `metadata.userId`, wherever it was written from. Browsable at `/admin/audit`; retention `audit.retentionDays`; no secrets stored.
